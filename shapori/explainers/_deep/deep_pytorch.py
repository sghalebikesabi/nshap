...
class PyTorchDeep(Explainer):
    ...
    def shap_values(self, X, ranked_outputs=None, output_rank_order="max", check_additivity=False, neighbourhood=False, scaling=False, bandwidth=30., lime=False):
            ...
            for j in range(X[0].shape[0]):
                ...
                if self.interim:
                    ...
                    for l in range(len(X)):
                        notmean_phis = (torch.from_numpy(sample_phis[l][self.data[l].shape[0]:]).to(self.device) * (X[l][j: j + 1] - self.data[l])).cpu().detach().numpy()
                        data_shape = self.data[l].shape
                        if self.neighbourhood or self.lime:
                            if self.scaling:
                                scaler = StandardScaler()
                                scaled = scaler.fit_transform(self.data[l].view((data_shape[0], -1)))
                                incoming_instance_ = scaler.transform(X[l][j: j + 1].view((1, -1)))
                            else: 
                                incoming_instance_ = X[l][j: j + 1].view((1, -1))
                                if self.lime:
                                    scaled = tiled_X[l].view((data_shape[0], -1))
                                else:
                                    scaled = self.data[l].view((data_shape[0], -1))

                            distances = ((scaled - incoming_instance_)**2).mean(-1)
                            dist_weights = np.exp(-(distances/(self.bandwidth**2)))
                            dist_weights /= dist_weights.sum()*len(dist_weights)
                            dist_weights *= len(dist_weights)
                            if (dist_weights != dist_weights).any():
                                raise ValueError('Choose higher bandwidth.')
                            # print(pd.DataFrame(dist_weights).describe())
                            notmean_phis = np.einsum('i, iljk -> ljk', dist_weights, notmean_phis)
                        phis[l][j] = notmean_phis.mean(0)
