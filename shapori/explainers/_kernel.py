...
class Kernel(Explainer):
    ...
    def __init__(self, model, data, link=IdentityLink(), **kwargs):
        ...
        assert isinstance(self.data, DenseData) or isinstance(
            self.data, SparseData
        ), "Shap explainer only supports the DenseData and SparseData input currently."
        assert (
            not self.data.transposed
        ), "Shap explainer does not support transposed DenseData or SparseData currently."
    ...
    def shap_values(self, X, **kwargs):
        ...
        if sp.sparse.issparse(X) and not sp.sparse.isspmatrix_lil(X):
            X = X.tolil()
        assert x_type.endswith(arr_type) or sp.sparse.isspmatrix_lil(X), (
        ...
        if len(X.shape) == 1:
            ...
            # vector-output
            s = explanation.shape
            if len(s) == 2 and not self.dist_per_coal:
                outs = [np.zeros(s[0]) for j in range(s[1])]
                for j in range(s[1]):
                    outs[j] = explanation[:, j]
                return outs

            elif len(s) == 3 and self.dist_per_coal:  # TODO
                outs = [np.zeros(s[0]) for j in range(s[1])]
                for j in range(s[1]):
                    outs[j] = explanation[:, j]
                return outs
    ...
    def explain(self, incoming_instance, **kwargs):
        ...
        # compute weights if locality is desired
        nbrh = kwargs.get("nbrh")
        self.dist_per_coal = kwargs.get("dist_per_coal")
        if nbrh:
            self.bandwidth = kwargs.get("bandwidth", np.array((0.75 * np.sqrt(self.M))))
            if isinstance(self.bandwidth, float):
                self.bandwidth = np.array([self.bandwidth])
            self.num_bw = len(self.bandwidth)
            distances = np.sqrt(((self.data.data - incoming_instance) ** 2).sum(1))
            self.incoming_instance = incoming_instance
            arg_w = -(distances ** 2)
            weights = np.exp(
                arg_w.reshape((1, len(self.data.data)))
                / self.bandwidth.reshape((-1, 1))
            )
            self.data.weights = weights / weights.sum(1).reshape((-1, 1))
            # self.data.weights /= sum(self.data.weights)

            # fnull changes if nbrh
            self.fnull = np.sum(
                (self.model_null.T.reshape((1, -1)) * self.data.weights).T, 0
            )
            if len(self.fnull.shape) == 1:
                self.fnull = np.array([self.fnull])
                # self.expected_value = float(self.expected_value)
        else:
            self.num_bw = 1
            self.data.weights = self.data.weights.reshape((1, -1))

        # if no features vary then no feature has an effect
        if self.M == 0:
            phi = np.zeros((self.num_bw, self.data.groups_size, self.D))
            phi_var = np.zeros((self.num_bw, self.data.groups_size, self.D))

        # if only one feature varies then it has all the effect
        elif self.M == 1:
            phi = np.zeros((self.num_bw, self.data.groups_size, self.D))
            phi_var = np.zeros((self.num_bw, self.data.groups_size, self.D))
            diff = self.link.f(self.fx).reshape((self.D, 1)) - self.link.f(self.fnull)
        ...
        # if more than one feature varies then we have to do real work
            ...
            phi = np.zeros((self.data.groups_size, self.D, self.num_bw))
            # phi_var = np.zeros((self.data.groups_size, self.D, self.num_bw))
            for d in range(self.D):
    ...
    def run(self):
        ...
        if self.dist_per_coal:
            resh_data = data.reshape((self.nsamplesAdded, self.N, data.shape[-1]))
            distances = np.sqrt(((resh_data - self.incoming_instance) ** 2).sum(-1))
            arg_w = -(distances ** 2)
            weights = np.exp(
                arg_w[np.newaxis, ...] / self.bandwidth.reshape((-1, 1, 1))
            )
            norm_weights = weights / weights.sum(-1)[..., np.newaxis]

        # find the expected value of each output
        self.ey = np.zeros((self.nsamples, self.D, self.num_bw))
        for i in range(self.nsamplesRun, self.nsamplesAdded):
            eyVal = np.zeros((self.D, self.num_bw))
            if self.dist_per_coal:
                self.data.weights = norm_weights[:, i, :]

            for j in range(0, self.N):
                eyVal += (
                    self.y[i * self.N + j, :][..., np.newaxis] * self.data.weights[:, j]
                )
    ...
    def solve(self, fraction_evaluated, dim):
        ...
        if (self.l1_reg not in ["auto", False, 0]) or (
            ...
            log.info(
                "np.sum(self.kernelWeights) = {0}".format(np.sum(self.kernelWeights))
            )
            w_sqrt_aug = np.sqrt(w_aug)
            eyAdj_aug = np.concatenate(
                (
                    eyAdj,
                    eyAdj - (self.link.f(self.fx[dim]) - self.link.f(self.fnull[dim])),
                )
            )
            eyAdj_aug *= w_sqrt_aug[..., np.newaxis]
            mask_aug = np.transpose(
                w_sqrt_aug
                * np.transpose(np.vstack((self.maskMatrix, self.maskMatrix - 1)))
            )
            ...
            # use an adaptive regularization method
                ...
                nonzero_inds = np.nonzero(
                    LassoLarsIC(criterion=c).fit(mask_aug, eyAdj_aug).coef_
                )[0]

            # use a fixed regularization coeffcient
            else:
                nonzero_inds = np.nonzero(
                    Lasso(alpha=self.l1_reg).fit(mask_aug, eyAdj_aug).coef_
                )[0]
        ...
        eyAdj2 = eyAdj - self.maskMatrix[:, nonzero_inds[-1]][..., np.newaxis] * (
        ...
        log.debug(
            "self.link(self.fx) - self.link(self.fnull) = {0}".format(
                self.link.f(self.fx[dim]) - self.link.f(self.fnull[dim])
            )
        )
        ...
        phi = np.zeros((self.M, self.num_bw))
