...
class Kernel(Explainer):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.add_marginal = add_marginal
        self.neighbourhood = neighbourhood
        self.bandwidth = bandwidth
        self.imp_sample = imp_sample
        self.scaling = scaling
        self.measure_missing = measure_missing
        self.dist_kernel = dist_kernel

        if self.interactions:
            assert self.conditional
        if self.add_marginal:
        ...
        # find E_x[f(x)]
        if isinstance(self.model_null, (pd.DataFrame, pd.Series)):
            self.model_null = np.squeeze(self.model_null.values)
        if safe_isinstance(self.model_null, "tensorflow.python.framework.ops.EagerTensor"):
            self.model_null = self.model_null.numpy()
    ...
    def explain(self, incoming_instance, **kwargs):
        ...
        # if only one feature varies then it has all the effect
            ...
            if self.neighbourhood:
                if self.scaling:
                    ...
                    scaled = scaler.fit_transform(self.data.data)
                    incoming_instance_ = scaler.transform(incoming_instance)
                else:
                    scaled = self.data.data
                    incoming_instance_ = incoming_instance

                distances = np.sum((scaled - incoming_instance_)**2, -1)
                if self.dist_kernel is None:
                    dist_weights = np.exp(-(distances/(self.bandwidth**2)))
                else:
                    dist_weights = (distances < self.bandwidth)*1.
                dist_weights /= dist_weights.sum()


                self.data.weights = dist_weights
                self.fnull = np.sum((self.model_null.T * self.data.weights).T, 0)
                if len(self.fnull.shape) == 0:
                    self.fnull = np.array([self.fnull])
        ...
        # if more than one feature varies then we have to do real work
            ...
            ## weight vector is list of weights of subsets of size s and M-1-s, sampling s features has weight (M-1)/(s(M-s))
            num_subset_sizes = np.int(np.ceil((self.M - 1) / 2.0))
            num_paired_subset_sizes = np.int(np.floor((self.M - 1) / 2.0))
            weight_vector = np.array([1/(self.M-i) for i in range(1, num_subset_sizes + 1)])
            weight_vector[:num_paired_subset_sizes] *= 2
            weight_vector /= np.sum(weight_vector)/(1-1/self.M)
            ...
            if num_full_subsets != num_subset_sizes:
                ...
                self.nfixed_samples = nfixed_samples

            # execute the model on the synthetic samples we have created
            self.run(incoming_instance)

            # solve then expand the feature importance (Shapley value) vector to contain the non-varying features
            if self.interactions:
                phi = np.zeros((self.data.groups_size + self.data.groups_size*(self.data.groups_size-1)//2, self.D))
                phi_var = np.zeros((self.data.groups_size + self.data.groups_size*(self.data.groups_size-1)//2, self.D))
            else:
                phi = np.zeros((self.data.groups_size, self.D))
                phi_var = np.zeros((self.data.groups_size, self.D))
            for d in range(self.D):
                vphi, vphi_var = self.solve(self.nsamples / self.max_samples, d)
                phi[self.varyingInds, d] = vphi[:self.M]
                phi_var[self.varyingInds, d] = vphi_var[:self.M]
                if self.interactions:
                    phi[self.M:, d] = vphi[self.M:]
                    phi_var[self.M:, d] = vphi_var[self.M:]
    ...
    def allocate(self):
        if sp.sparse.issparse(self.data.data):
            ...
            self.mask = np.ones_like(self.synth_data)
        ...
        if not self.new_sampler:
            ...
            if self.add_marginal:
                self.ey_2 = np.zeros((self.nsamples, self.D))
        else:
            self.maskMatrix = np.zeros((self.nsamples+2, self.M))
            self.maskMatrix[-1] = 1
            self.kernelWeights = np.zeros(self.nsamples+2)
            self.ey = np.zeros((self.nsamples+2, self.D))
            self.ey[-2] = self.fnull
            self.ey[-1] = self.fx

        if self.conditional:
            self.imp = IterativeImputer(max_iter=100, random_state=0, sample_posterior=True)
            self.imp.fit(self.data.data)
            self.synth_data = np.zeros_like(self.synth_data) + np.nan 

        self.y = np.zeros((self.nsamples * self.N, self.D))
        if self.add_marginal:
            self.y_2 = np.zeros((self.nsamples * self.N, self.D))
        ...
        if isinstance(self.varyingFeatureGroups, (list,)):
            for j in range(self.M):
                for k in self.varyingFeatureGroups[j]:
                    if m[j] == 1.0:
                        ...
                        self.mask[offset:offset+self.N, k] = 0
        else:
            ...
            if len(groups.shape) == 2:
                for group in groups:
                    ...
                    self.mask[offset:offset+self.N, group] = 0
            else:
                ...
                self.mask[offset:offset+self.N, groups] = 0
        self.maskMatrix[self.nsamplesAdded, :] = m
        self.kernelWeights[self.nsamplesAdded] = w
        self.nsamplesAdded += 1

    def run(self, incoming_instance):
        num_to_run = self.nsamplesAdded * self.N - self.nsamplesRun * self.N
        if self.conditional:
            mask = np.isnan(self.synth_data)
            self.synth_data = self.imp.transform(self.synth_data)
            if self.add_marginal:
                self.synth_data_2 = (1-mask)*self.synth_data.copy() + mask*np.tile(self.data.data.copy(), (self.nsamples, 1))
        ...
        if self.add_marginal:
            data_2 = self.synth_data_2[self.nsamplesRun*self.N:self.nsamplesAdded*self.N,:]
            if self.keep_index:
                index_2 = self.synth_data_index[self.nsamplesRun*self.N:self.nsamplesAdded*self.N]
                index_2 = pd.DataFrame(index_2, columns=[self.data.index_name])
                data_2 = pd.DataFrame(data_2, columns=self.data.group_names)
                data_2 = pd.concat([index_2, data_2], axis=1).set_index(self.data.index_name)
                if self.keep_index_ordered:
                    data_2 = data_2.sort_index()

            modelOut_2 = self.model.f(data_2)
            if isinstance(modelOut_2, (pd.DataFrame, pd.Series)):
                modelOut_2 = modelOut_2.values
            self.y_2[self.nsamplesRun * self.N:self.nsamplesAdded * self.N, :] = np.reshape(modelOut_2, (num_to_run, self.D))
        ...
        if self.neighbourhood:
            ...
            print(pd.DataFrame(dist_weights).describe())

        if self.measure_missing:
            distances = ((data - incoming_instance)**2).sum(1)
            distances *= self.mask.shape[1]/(self.mask.shape[1] - self.mask.sum(1))
            if self.dist_kernel is None:
                dist_weights = np.exp(-(distances/(self.bandwidth**2)))
            else:
                dist_weights = (distances < self.bandwidth)*1.
            ...
            dist_weights = self.data.weights

        print(pd.DataFrame(dist_weights))
        ...
        for i in range(self.nsamplesRun, self.nsamplesAdded):
            ...
            for j in range(0, self.N):
                ...
                # distances = np.sum((X_test - local_obs)**2, -1)
                # dist_weights = np.exp(-distances/std_sigma**2)
                # dist_weights /= dist_weights.sum()

                eyVal += self.y[i * self.N + j, :] * dist_weights[j]
            self.ey[i, :] = eyVal

            if self.add_marginal:
                eyVal_2 = np.zeros(self.D)
                for j in range(0, self.N):
                    eyVal_2 += self.y_2[i * self.N + j, :] * self.data.weights[j]

                self.ey_2[i, :] = eyVal_2

            self.nsamplesRun += 1

    def solve(self, fraction_evaluated, dim, new_sampler=False):
        
        if self.interactions:
            new_mask = np.zeros((len(self.maskMatrix), self.M + (self.M-1)*(self.M)//2))
            new_mask[:, :self.M] = self.maskMatrix
            new_mask_2 = new_mask.copy()
            curr_ix = 0
            for i in range(self.M-1):
                for j in range(i+1, self.M):
                    new_mask[(self.maskMatrix[:, i]==1) & (self.maskMatrix[:, j]==0), self.M + curr_ix] = 1
                    new_mask[(self.maskMatrix[:, i]==0) & (self.maskMatrix[:, j]==1), self.M + curr_ix] = 1
                    curr_ix += 1
            self.maskMatrix = new_mask
            if self.add_marginal:
                self.maskMatrix = np.concatenate([new_mask, new_mask_2], 0)
                self.kernelWeights = np.tile(self.kernelWeights, (2))/2

        s = np.sum(self.maskMatrix, 1)
        w_with = 1/(binom(self.M-1, s-1)*self.M)
        w_with[-2] = w_with[-1]
        # w_with = self.kernelWeights*(self.M-s)
        # scale_fac_with_c = w_with[0]*(binom(self.M-1, s[0]-1)*self.M)

        if not self.new_sampler:
            eyAdj = self.linkfv(self.ey[:, dim]) - self.link.f(self.fnull[dim])
            if self.add_marginal:
                eyAdj_2 = self.linkfv(self.ey_2[:, dim]) - self.link.f(self.fnull[dim])
                eyAdj = np.concatenate([eyAdj, eyAdj_2], 0)
            ...
            # do feature selection if we have not well enumerated the space
            if self.interactions:
                nonzero_inds = np.arange(self.M + (self.M-1)*(self.M)//2)
            else:
                nonzero_inds = np.arange(self.M)
            ...
            if self.interactions: 
                phi = np.zeros(self.M + (self.M-1)*(self.M)//2)
            else:
                phi = np.zeros(self.M)
            ...
            if not self.new_sampler:
                eyAdj0 = np.zeros((self.nsamples+2))
                eyAdj0[:-2] = self.linkfv(self.ey[:, dim])
                eyAdj0[-1] = self.link.f(self.fx[dim])
                eyAdj0[-2] = self.link.f(self.fnull[dim])
                maskMatrix = np.zeros((self.nsamples+2, self.M))
                maskMatrix[:-2] = self.maskMatrix[:self.nsamples]
                maskMatrix[-1] = 1
                self.maskMatrix = maskMatrix
                s = np.sum(self.maskMatrix, 1)
                w_without = np.ones((self.nsamples+2))/(self.M-s)
                w_with = np.ones((self.nsamples+2))/s
                # w_with = w_without*s//(self.M-s)
                w_without[-1] = w_without[-2]
                w_with[-2] = w_without[-1]
                w_without[:-2] /= sum(w_without[:-2])/(1-1/self.M)
                w_with[:-2] /= sum(w_with[:-2])/(1-1/self.M)
                # w_with = np.ones((self.nsamples+2))
                # w_with[:-2] = self.kernelWeights*(self.M-s)/(self.M-1)
                # w_with[-2:] = 1/self.M
                # w_without = np.ones((self.nsamples+2))
                # w_without[:-2] = w_with[:-2]/(self.M-s)*s
                # w_without[-2:] = 1/self.M
                # scale_fac_with_c = w_with[0]*(binom(self.M-1, s[0]-1)*self.M)
                # w_with[-2:] = scale_fac_with_c/self.M
                # w_without[-2:] = w_without[0]*w_with[-1]/w_with[0]/s[0]*(self.M-s[0])
            if not self.new_sampler: # new idea where we count the subsets
                from collections import Counter
                c = Counter(s)
                # double_c = {k: (c[k]+c[self.M-k])//2 for k in c.keys()}
                # num_subset_sizes = len(c)-1
                s_weights = np.array([1/c[k] for k in s])
                w_without = s_weights/(self.M-s)
                w_with = s_weights/s
                w_without[-1] = w_without[-2]
                w_with[-2] = w_without[-1]
                w_with[:-2] /= sum(w_with)*2

            eyAdj0 = self.linkfv(self.ey[:, dim])
            w_without = w_with/(self.M-s)*s
            w_without[-2:] = w_with[-1]

            # m1maskMatrix = 1 - self.maskMatrix
            # with_M = np.einsum('i, ij -> j', w_with, self.maskMatrix)
            # without_M = np.einsum('i, ij -> j', w_without, 1 - self.maskMatrix)
            with_M_y = np.einsum('i, i, ij -> j', w_with, eyAdj0, self.maskMatrix)
            without_M_y = np.einsum('i, i, ij -> j', w_without, eyAdj0, 1 - self.maskMatrix)
            # if self.new_sampler=='average':
            #     phi = with_M_y/with_M - without_M_y/without_M
            # elif self.new_sampler=='adds_up':
            phi = with_M_y - without_M_y
            # elif self.new_sampler=='better_average':
            #     with_M_y_ = np.einsum('i, i, ij -> j', w_with[:-2], eyAdj0[:-2], self.maskMatrix[:-2])
            #     without_M_y_ = np.einsum('i, i, ij -> j', w_without[:-2], eyAdj0[:-2], m1maskMatrix[:-2])
            #     phi = with_M_y - without_M_y + (1-with_M) * with_M_y_ - (1-without_M) * without_M_y_
            # elif self.new_sampler=='appr_average':
            #     with_M_y_ = np.einsum('i, i, ij -> j', w_with[:-2], eyAdj0[:-2], self.maskMatrix[:-2])
            #     without_M_y_ = np.einsum('i, i, ij -> j', w_without[:-2], eyAdj0[:-2], m1maskMatrix[:-2])
            #     phi = with_M_y - without_M_y + (1-with_M) * (with_M_y_ - without_M_y_)

            # phi = [
            #     (np.sum(w_with*self.maskMatrix[:, j]*eyAdj0))/(sum(w_with*maskMatrix[:, j])) - (np.sum(w_without*m1maskMatrix[:, j]*eyAdj0))/(sum(w_without*m1maskMatrix[:, j])) for j in range(self.M)
            # ]
